...
describe('FrequencyController', () => {
  ...
  beforeEach(() => {
    controller = new FrequencyController(1000, 3000);
  });

  describe('constructor', () => {
    it('应该使用默认延迟范围创建实例', () => {
      const defaultController = new FrequencyController();
    ...
    it('应该使用自定义延迟范围创建实例', () => {
      const customController = new FrequencyController(2000, 5000);
  ...
  describe('setDelayRange', () => {
    it('应该正确设置延迟范围', () => {
      controller.setDelayRange(1500, 4000);
  ...
  describe('randomDelay', () => {
    ...
    it('应该处理最小延迟大于最大延迟的情况', async () => {
      const controller = new FrequencyController(200, 100);
      const start = Date.now();
      await controller.randomDelay();
      const end = Date.now();
